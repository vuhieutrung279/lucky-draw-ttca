...
/**
 * Real-time synchronization plugin for Lucky Draw across multiple screens and laptops.
 * Relays events sent over Vite's HMR WebSocket to all connected browser clients on the network.
 */
function luckyDrawSyncPlugin(): Plugin {
  return {
    name: 'luckydraw-sync-relay',
    configureServer(server) {
      server.ws.on('luckydraw:sync', (data, client) => {
        // Broadcast message to all connected clients on LAN
        server.ws.send({
          type: 'custom',
          event: 'luckydraw:sync-relay',
          data,
        });
      });
    },
